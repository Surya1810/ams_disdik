                    ...
                    <form id="addSekolahForm" action="{{ route('sekolah.store') }}" method="POST">
                        ...
                        <div class="card-body mb-3">
                            ...
                            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                                required placeholder="Masukkan nama sekolah" autofocus aria-label="Name">
                            @error('name')
                            <span class="invalid-feedback"><strong>{{ $message }}</strong></span>
                            @enderror
            ...
            <div class="modal-body p-0">
                <div class="card card-plain">
                    ...
                    <form id="editSekolahForm" method="POST">
                        ...
                        <!-- Header Modal -->
                        <div class="card-header pb-0 text-left">
                            <h4 class="text-primary text-gradient">Edit <strong>Sekolah</strong></h4>
                        </div>
                        ...
                        <div class="card-body mb-3">
                            ...
                            <select id="edit_category" name="category" required class="form-control">
                                <option value="">-- Pilih Kategori --</option>
                                <option value="SD">SD</option>
                                <option value="SMP">SMP</option>
                                <option value="SMA">SMA</option>
                            ...
                            <!-- Input Nama Sekolah -->
                            <label for="edit_name" class="form-label mt-3">Nama Sekolah</label>
                            <input type="text" id="edit_name" name="name" required placeholder="Masukkan nama sekolah"
                                class="form-control">

                            <!-- Input Kecamatan -->
                            <label for="edit_kecamatan_id" class="form-label mt-3">Kecamatan</label>
                            <select id="edit_kecamatan_id" name="kecamatan_id" required class="form-control">
                                @foreach($kecamatans as $kecamatan)
                                <option value="{{ $kecamatan->id }}">{{ $kecamatan->name }}</option>
                                @endforeach
                            </select>
...
<script type='text/javascript'>
    ...
    function editSekolah(id, name, kecamatan_id, category) {
        ...
        var actionUrl = "{{ route('sekolah.update', ':id') }}".replace(':id', id);
