                    ...
                    <form action="{{ route('kecamatan.store') }}" method="POST">
                        ...
                        <div class="card-body mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" name="name"
                                value="{{ old('name') }}" required placeholder="Nama Kecamatan" autofocus
                                aria-label="Name">
                            @error('name')
                ...
                <div class="card card-plain">
                    <form id="editKecamatanForm" action="{{ route('kecamatan.update', 'kecamatan_id') }}" method="POST">
                        ...
                        <div class="card-body mb-3">
                            <label>Nama Kecamatan</label>
                            <input type="text" class="form-control @error('name') is-invalid @enderror" name="name"
                                id="edit_name" value="{{ old('name') }}" required placeholder="Nama Kecamatan" autofocus
                                aria-label="Name">
                            @error('name')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                        ...
                        <div class="card-footer text-center pt-0 px-lg-2 px-1">
                            <button type="submit" class="btn btn-warning rounded-partner m-0">Update</button>
                        </div>
                    </form>
                </div>
            </div>
